...
- 🧵 **Assign filament from a phone** — Open `/spoolbuddy/assign`, install it as a focused PWA if desired, scan a printer label and spool label, pick the AMS or external holder slot, and Bambuddy applies the existing inventory or Spoolman slot-assignment workflow.
...
**Personal fork on Unraid:**

This fork includes a GitHub Actions workflow that publishes a GHCR image from `main`.
See [Unraid Deployment From A Personal Fork](docs/unraid-fork-deploy.md) for switching
an existing Unraid Community Apps install to your fork's image.
